...
class Stock extends CI_Model{
    ...
    function add_opening_stock($guid,$item,$quty,$cost,$sell,$per,$dis,$tax,$net,$supplier){
         ...
         // add stock 
           $price=$sell;
            $this->db->select()->from('stock')->where('branch_id',$this->session->userdata('branch_id'))->where('item',$item)->where('price',$sell);
    ...
    function search_items($search){
     ...
     function approve_order($guid){
         $this->db->where('guid',$guid);
         $this->db->update('opening_stock',array('order_status'=>1));
        
     }
     function  check_approve($guid){
          $this->db->select()->from('opening_stock')->where('guid',$guid)->where('order_status',1);
            $sql=  $this->db->get();
            if($sql->num_rows()>0){
               return FALSE;
            }else{
