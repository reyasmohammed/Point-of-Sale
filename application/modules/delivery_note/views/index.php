...
<script type="text/javascript">
        ...
        var unicode=e.charCode? e.charCode : e.keyCode
            if (unicode!=13 && unicode!=9){          
             ...
             if (unicode!=27){
                     ...
                     $('#parsley_reg #delivery_note_date').focus();
    ...
    function receive_free_items_update(i)
    {
        if(isNaN($('#parsley_reg #r_free_id_'+i).val())){
           $('#parsley_reg #r_free_id_'+i).val("");
            ...
            var good=  $('#parsley_reg #r_free_id_'+i).val();
            var res=  $('#parsley_reg #o_free_id_'+i).val();
             var old=  $('#parsley_reg #delivery_note_old_free_'+i).val();
            if(parseFloat(good)>parseFloat(res)){
                $('#parsley_reg #r_free_id_'+i).val(res);
    ...
    if($('#parsley_reg #delivery_note_guid').val()!=""){
   ...
   if($('#parsley_reg #order_date').value!=""){
    ...
    function save_new_delivery_note(){
         <?php if($this->session->userdata['delivery_note_per']['add']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?php echo base_url('index.php/delivery_note/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('delivery_note').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_delivery_note_lists();
                                       refresh_items_table();
                                          ...
                                          $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('delivery_note');?>', { type: "error" });                           
                                    }
                       }
                });
                    }else{
                     ...
                     $('#delivery_note_no').val(delivery_note_number);
                     $('#demo_delivery_note_no').val(delivery_note_number);
                    }
                    }else{
    ...
    function update_order(){
         <?php if($this->session->userdata['delivery_note_per']['edit']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?php echo base_url('index.php/delivery_note/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('delivery_note').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_delivery_note_lists();
                                       refresh_items_table();
    ...
    return  "<p >"+sup.text+"<img src='<?php echo base_url() ?>/uploads/items/"+sup.image+"' style='float:right;height:59px'></img></p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.value+"</p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.category+"</p> <p style='width:130px;  margin-left: 218px'> "+sup.brand+"</p><p style='width:120px;  margin-left: 380px;margin-top: -28px;'> "+sup.department+"</p>";
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?php echo base_url() ?>index.php/delivery_note/search_items/',
            ...
            if($('#parsley_reg #demo_order_number').select2('data').expired==0){
             ...
             $('#loading').modal('show');
                   var guid = $('#parsley_reg #demo_order_number').select2('data').id;
                            ...
                            $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/delivery_note/get_sales_order/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                             success: function(data)        
                             { 
                             if(data[0]['delivery_note_status']==0){
                                 
                                $('#parsley_reg #delivery_note_guid').val($('#parsley_reg #demo_order_number').select2('data').id);
                      ...
                      window.setTimeout(function ()
                    {
                     ...
                     $('#parsley_reg #demo_order_number').select2('open');
                     $("#parsley_reg").trigger('reset');
                     $.bootstrapGrowl('<?php echo $this->lang->line('sales_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?php echo $this->lang->line('was_expired');?>', { type: "error" });                         
                     $('#delivery_note_no').val(delivery_note_number);
                     $('#demo_delivery_note_no').val(delivery_note_number);
          ...
          $('#parsley_reg #demo_order_number').select2({
              dropdownCssClass : 'supplier_select',
               formatResult: format_sales_order,
                ...
                ajax: {
                     url: '<?php echo base_url() ?>index.php/delivery_note/search_sales_order',
                     data: function(term, page) {
                            return {types: ["exercise"],
                                limit: -1,
                                term: term
...
refresh_items_table();
   $("#parsley_reg").trigger('reset');
    <?php if($this->session->userdata['delivery_note_per']['add']==1){ ?>
            ...
            $(".porchase_order_for_delivery_note").hide();
            $('#save_button').show();
            $('#update_clear').hide();
            $('#save_clear').show();
            $('#total_amount').val('');
            ...
            $("#parsley_reg #demo_order_number").select2('data', {id:'',text: 'Search PO'});
             $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/delivery_note/order_number/",                      
                             data: "", 
                             dataType: 'json',               
                             success: function(data)        
                             {    
                                 ...
                                 $('#parsley_reg #demo_delivery_note_no').val(data[0][0]['prefix']+data[0][0]['max']);
                                 $('#parsley_reg #delivery_note_no').val(data[0][0]['prefix']+data[0][0]['max']);
                                 delivery_note_number=data[0][0]['prefix']+data[0][0]['max'];
                             }
                             });
...
function posnic_delivery_note_lists(){
      ...
      $('#posnic_add_delivery_note').removeAttr("disabled");
      $('#delivery_note_lists').attr("disabled",'disabled');
}
function clear_add_delivery_note(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
}
function clear_update_delivery_note(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
      edit_function($('#delivery_note_guid').val());
}
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_delivery_note" class="btn btn-default" ><i class="icon icon-user"></i> <?php echo $this->lang->line('addnew') ?></a>  
                        ...
                        <a href="javascript:delivery_note_group_delete()" class="btn btn-default" id="delete"><i class="icon icon-trash"></i> <?php echo $this->lang->line('delete') ?></a>
                        <a href="javascript:posnic_delivery_note_lists()" class="btn btn-default" id="delivery_note_lists"><i class="icon icon-list"></i> <?php echo $this->lang->line('delivery_note') ?></a>
...
<section class="container clearfix main_section">
        <div id="main_content_outer" class="clearfix">
            <div id="main_content">
                    ...
                    echo form_open('delivery_note/delivery_note_manage',$form) ?>
                        <div class="row">
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?php echo $this->lang->line('delivery_note') ?></h4>                                                                               
                                    </div>
                                        ...
                                        <tr>
                                         <th>Id</th>
                                          ...
                                          <th ><?php echo $this->lang->line('sales_order') ?></th>
                                           <th style="width: 170px !important"><?php echo $this->lang->line('delivery_note_number') ?></th>
...
<section id="add_new_order" class="container clearfix main_section">
     <?php   $form =array('id'=>'parsley_reg',
       ...
       echo form_open_multipart('delivery_note/upadate_pos_delivery_note_details/',$form);?>
    ...
    <div id="main_content" style="padding: 0 14px !important;">
        ...
        <input type="hidden" name="delivery_note_guid" id="delivery_note_guid" >
                         <div class="row">
                          <div class="panel panel-default">
                              <div class="panel-heading" >
                                     <h4 class="panel-title"><?php echo $this->lang->line('delivery_note')." ".$this->lang->line('details') ?></h4>                                                                               
                               </div>
                                       ...
                                       <div id="" class="col col-sm-12" style="padding-right: 25px;padding-left: 25px">
                                           <div class="row">
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep supplier_select_2">
                                                        ...
                                                        <input type="hidden" id="delivery_note_guid" name="delivery_note_guid">
                                                       
                                                  </div> 
                                                   <div class="form_sep porchase_order_for_delivery_note" style="margin-top:0px">
                                                         <label for="demo_order_number" ><?php echo $this->lang->line('order_number') ?></label>	
                                                         <input type="text" disabled="disabled" id="edit_delivery_note_node" class='form-control'>
                                                   </div>
                                               </div>
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                           ...
                                           <div class="row">
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="delivery_note_no" ><?php echo $this->lang->line('delivery_note_no') ?></label>													
                                                                     <?php $round_off_amount=array('name'=>'demo_delivery_note_no',
                                                                                        'class'=>'  form-control',
                                                                                        'id'=>'demo_delivery_note_no',
                                                                                        'disabled'=>'disabled',
                                                                                        'value'=>set_value('demo_delivery_note_no'));
                                                                         echo form_input($round_off_amount)?>
                                                            <input type="hidden" name="delivery_note_no" id="delivery_note_no">
                                                       </div>
                                                    </div>
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="delivery_note_date" ><?php echo $this->lang->line('delivery_note_date') ?></label>													
                                                                     <div class="input-group date ebro_datepicker" data-date-format="dd.mm.yyyy" data-date-autoclose="true" data-date-start-view="2">
                                                                           <?php $delivery_note_date=array('name'=>'delivery_note_date',
                                                                                            'class'=>'required form-control',
                                                                                            'id'=>'delivery_note_date',
                                                                                            'onKeyPress'=>"new_order_date(event)", 
                                                                                            'value'=>set_value('delivery_note_date'));
                                                                             echo form_input($delivery_note_date)?>
                                                                <span class="input-group-addon"><i class="icon-calendar"></i></span>
                                                                </div>
                                                       </div>
                                                   </div>
                                      ...
                                      <div class="row">
                                          <div class="col col-sm-3" style="padding-top: 50px" >
                                              <div class="form_sep " id="save_button" style="padding-left: 50px">
                                                       <label for="" >&nbsp;</label>	
                                                       <a href="javascript:save_new_delivery_note()" class="btn btn-default"  ><i class="icon icon-save"></i> <?php echo " ".$this->lang->line('save') ?></a>
                                                  </div>
                                          ...
                                          <div class="col col-sm-3" style="padding-top: 50px"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_delivery_note()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_delivery_note()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
    ...
    function delivery_note_group_delete(){
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?php echo $this->lang->line('Select Atleast One')."".$this->lang->line('delivery_note');?>', { type: "warning" });
                      }else{
                            bootbox.confirm("<?php echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('delivery_note') ?>", function(result) {
             if(result){
