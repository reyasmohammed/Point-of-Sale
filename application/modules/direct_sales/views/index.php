...
<script type="text/javascript">
    ...
    function save_new_order(){
         <?asp if($annan->session->userdata['direct_sales_per']['add']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/direct_sales/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $annan->lang->line('direct_sales').' '.$annan->lang->line('added');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_direct_sales_lists();
                                       refresh_items_table();
                                    }else  if(response['responseText']=='ALREADY'){
                                           $.bootstrapGrowl($('#parsley_reg #order_number').val()+' <?asp echo $annan->lang->line('customers').' '.$annan->lang->line('is_already_added');?>', { type: "warning" });                           
                                    }else  if(response['responseText']=='FALSE'){
                                           $.bootstrapGrowl('<?asp echo $annan->lang->line('Please Enter All Required Fields');?>', { type: "warning" });                           
                                    }else{
                                          $.bootstrapGrowl('<?asp echo $annan->lang->line('You Have NO Permission To Add')." ".$annan->lang->line('direct_sales');?>', { type: "error" });                           
                                    }
                       }
                });
                    }else{
                   ...
                   $.bootstrapGrowl('<?asp echo $annan->lang->line('You Have NO Permission To Add')." ".$annan->lang->line('customers');?>', { type: "error" });                       
                    <?asp }?>
    }
    function update_order(){
         <?asp if($annan->session->userdata['direct_sales_per']['edit']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/direct_sales/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $annan->lang->line('direct_sales').' '.$annan->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_direct_sales_lists();
                                       refresh_items_table();
    ...
    function save_sales_bill(){
         <?asp if($annan->session->userdata['direct_sales_per']['edit']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/direct_sales/save_sales_bill')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $annan->lang->line('save_sales_bill').' '.$annan->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_direct_sales_lists();
                                       refresh_items_table();
     ...
     $(document).ready( function () {
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/direct_sales/search_items/',
          ...
          $('#parsley_reg #first_name').select2({
              dropdownCssClass : 'customers_select',
               formatResult: format_customers,
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/direct_sales/search_customer',
                     data: function(term, page) {
                            return {types: ["exercise"],
                                limit: -1,
                                term: term
...
$("#parsley_reg #first_name").select2('data', {id:'',text: 'Search Customer'});
    <?asp if($annan->session->userdata['direct_sales_per']['add']==1){ ?>
             $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/direct_sales/order_number/",                      
                             data: "", 
                             dataType: 'json',               
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_direct_sales" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $annan->lang->line('addnew') ?></a>  
                     
                        <a href="javascript:direct_sales_group_approve()" class="btn btn-default" id="deactive"  ><i class="icon icon-play"></i> <?asp echo $annan->lang->line('approve') ?></a>
                        ...
                        <a href="javascript:posnic_direct_sales_lists()" class="btn btn-default" id="direct_sales_lists"><i class="icon icon-list"></i> <?asp echo $annan->lang->line('direct_sales') ?></a>
                            ...
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?asp echo $annan->lang->line('direct_sales') ?></h4>                                                                               
                                    </div>
                                    <table id="dt_table_tools" class="table-striped table-condensed" style="width: 100%"><thead>
                                        <tr>
                                         <th>Id</th>
                                          <th ><?asp echo $annan->lang->line('select') ?></th>
                                          <th ><?asp echo $annan->lang->line('order_number') ?></th>
...
function new_order_date(e){
 ...
 $.bootstrapGrowl('<?asp echo $annan->lang->line('Please_Select_A_Customer');?>', { type: "warning" }); 
         $('#parsley_reg #first_name').select2('open');
            ...
            if(isNaN($('#parsley_reg #stock_quty').val())){
             ...
             if(parseFloat($('#quantity').val())>parseFloat($('#stock_quty').val())){
               $('#quantity').val($('#stock_quty').val());
                $.bootstrapGrowl('<?asp echo $annan->lang->line('avilable_stock_is') ;?> '+$('#stock_quty').val(), { type: "warning" }); 
           }
    ...
    <div id="main_content" style="padding: 0 14px !important;">
        ...
        <input type="hidden" name="dummy_discount_amount" id="dummy_discount_amount" >
                         <div class="row">
                          <div class="panel panel-default">
                              <div class="panel-heading" >
                                     <h4 class="panel-title"><?asp echo $annan->lang->line('direct_sales')." ".$annan->lang->line('details') ?></h4>                                                                               
                               </div>
                                       ...
                                       <div id="" class="col col-sm-12" style="padding-right: 25px;padding-left: 25px">
                                           <div class="row">
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="order_number" ><?asp echo $annan->lang->line('direct_sales_number') ?></label>													
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="order_date" ><?asp echo $annan->lang->line('sales_date') ?></label>													
                                                ...
                                                <div class="col col-sm-2" id="sales_bill_number_div">
                                                   <div class="form_sep">
                                                            <label for="sales_bill_number" ><?asp echo $annan->lang->line('sales_bill_number') ?></label>													
                                                                     <?asp $order_number=array('name'=>'demo_sales_bill_number',
                                                                                        'class'=>'required  form-control',
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="discount" ><?asp echo $annan->lang->line('discount') ?>%</label>													
                                                                     <?asp $discount=array('name'=>'sales_discount',
                                                                                        'class'=>'  form-control',
                                                                                        'id'=>'id_discount',
                                           ...
                                           <div class="row">
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="customer_discount" ><?asp echo $annan->lang->line('customer').' '.$annan->lang->line('discount') ?> %</label>													
                                                                     <?asp $customer_discount=array('name'=>'customer_discount',
                                                                                        'class'=>'required  form-control',
                                                 ...
                                                 <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="customer_discount_amount" ><?asp echo $annan->lang->line('customer').' '.$annan->lang->line('disc').' '.$annan->lang->line('amt') ?></label>													
                                                                     <?asp $customer_discount_amount=array('name'=>'customer_discount_amount',
                                                                                        'class'=>'required  form-control',
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="sales_bill_date" ><?asp echo $annan->lang->line('sales_bill_date') ?></label>													
                                                                     <div class="input-group date ebro_datepicker" data-date-format="dd.mm.yyyy" data-date-autoclose="true" data-date-start-view="2">
                                                                           <?asp $sales_bill_date=array('name'=>'sales_bill_date',
                                                                                            'class'=>'required form-control',
                                                                                            'id'=>'sales_bill_date',
                                             ...
                                             <label for="items" class="text-center" ><?asp echo $annan->lang->line('items') ?></label>	
                                                 ...
                                                 <div class="col col-lg-1" style="padding:1px;width: 160px;">
                                                   ...
                                                   <div class="form_sep">
                                                                ...
                                                                <label for="tax" class="text-center" id="tax_label"  ><?asp echo $annan->lang->line('tax_type') ?></label>

                                                                 <?asp $tax_type=array('name'=>'tax_type',
                                                ...
                                                <div class="col col-lg-1" style="padding:1px;width:120px">
                                                   <div class="form_sep"> <label for="discount" class="text-center" ><?asp echo $annan->lang->line('discount') ?></label>
                    ...
                    <div class="col col-lg-9">
                            ...
                            <div class="image_items">
                                ...
                                <table id='selected_item_table' class="table table-striped dataTable ">
                                    <thead>
                                     ...
                                     <th><?asp echo $annan->lang->line('no') ?></th>
                                    <th><?asp echo $annan->lang->line('name') ?></th>
                        ...
                        <div class="row" style="margin-left: 5px">
                                     ...
                                     <h4 class="panel-title"><?asp echo $annan->lang->line('amount') ?></h4>                                                                               
                                                        ...
                                                        <label for="total_item_discount_amount" ><?asp echo $annan->lang->line('total_item_discount_amount') ?></label>													
                                                                  <?asp $total_item_discount_amount=array('name'=>'total_item_discount_amount',
                                                                                    'class'=>' form-control',
                                                         ...
                                                         <div class="form_sep " style="padding: 0 25px">
                                                        <label for="total_tax" ><?asp echo $annan->lang->line('total_tax') ?></label>													
                                                                  <?asp $total_item_discount_amount=array('name'=>'total_tax',
                                                                                    'class'=>' form-control',
                                                                                    'id'=>'total_tax',
                                                                                    'disabled'=>'disabled',
                        ...
                        <div class="row" style="margin-left: 5px" id="sales_buttons">
                                          ...
                                          <div class="col col-sm-6"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_direct_sales()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$annan->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_direct_sales()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$annan->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
                         ...
                         <div class="row" style="margin-left: 5px" id="sales_bill_buttons">
                                          <div class="col col-sm-6"  >
                                              <div class="form_sep " id="save_button" style="padding-left:0px">
                                                       <label for="" >&nbsp;</label>	
                                                       <a href="javascript:save_sales_bill()" class="btn btn-default"  ><i class="icon icon-save"></i> <?asp echo " ".$annan->lang->line('save') ?></a>
                                                  </div>
            ...
            <?asp if($annan->session->userdata['direct_sales_per']['delete']==1){ ?>
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?asp echo $annan->lang->line('Select Atleast One')."".$annan->lang->line('sales');?>', { type: "warning" });
                      }else{
                            bootbox.confirm("<?asp echo $annan->lang->line('Are you Sure To Delete')."".$annan->lang->line('sales') ?>", function(result) {
             if(result){
                          ...
                          if(posnic[i].checked==true){ 
                              ...
                              $.ajax({
                                ...
                                data: {
                                  ...
                                  complete: function(response) {
                                    if(response['responseText']=='TRUE'){
                                           $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $annan->lang->line('sales') ?>  <?asp echo $annan->lang->line('deleted');?>', { type: "error" });
                                        $("#dt_table_tools").dataTable().fnDraw();
                                         ...
                                         $.bootstrapGrowl('<?asp echo $annan->lang->line('You Have NO Permission')." ".$annan->lang->line('to')." ".$annan->lang->line('approve')." ".$annan->lang->line('sales');?>', { type: "error" });                       
                                    }
                                    }
                            });
                      ...
                      <?asp }else{?>
                                   $.bootstrapGrowl('<?asp echo $annan->lang->line('You Have NO Permission To Delete')." ".$annan->lang->line('sales');?>', { type: "error" });                       
                           <?asp }
                        ?>
                      }
    ...
    function direct_sales_group_approve(){
         <?asp if($annan->session->userdata['direct_sales_per']['approve']==1){ ?>
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                                               $.bootstrapGrowl('<?asp echo $annan->lang->line('Select Atleast One')."".$annan->lang->line('direct_sales');?>', { type: "warning" });
