...
<script type="text/javascript">
    ...
    function save_new_order(){
         <?asp if($this->session->userdata['purchase_order_per']['add']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/purchase_order/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       ...
                                       refresh_items_table();
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('purchase_order');?>', { type: "error" });                           
                                    }
                       }
                });
                    }else{
    ...
    function update_order(){
         <?asp if($this->session->userdata['purchase_order_per']['edit']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/purchase_order/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_purchase_order_lists();
                                       refresh_items_table();
    ...
    return  "<p >"+sup.text+"<img src='<?asp echo base_url() ?>/uploads/items/"+sup.image+"' style='float:right;height:59px'></img></p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.value+"</p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.category+"</p> <p style='width:130px;  margin-left: 218px'> "+sup.brand+"</p><p style='width:120px;  margin-left: 380px;margin-top: -28px;'> "+sup.department+"</p>";
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/purchase_order/search_items/',
          ...
          $('#parsley_reg #first_name').select2({
              dropdownCssClass : 'supplier_select',
               formatResult: format_supplier,
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/purchase_order/search_supplier',
                     data: function(term, page) {
                            return {types: ["exercise"],
                                limit: -1,
                                term: term
...
$("#parsley_reg #first_name").select2('data', {id:'',text: 'Search Supplier'});
    <?asp if($this->session->userdata['purchase_order_per']['add']==1){ ?>
             $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/purchase_order/order_number/",                      
                             data: "", 
                             dataType: 'json',               
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_purchase_order" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $this->lang->line('addnew') ?></a>  
                     
                        <a href="javascript:purchase_order_group_approve()" class="btn btn-default" id="deactive"  ><i class="icon icon-play"></i> <?asp echo $this->lang->line('approve') ?></a>
                        ...
                        <a href="javascript:posnic_purchase_order_lists()" class="btn btn-default" id="purchase_order_lists"><i class="icon icon-list"></i> <?asp echo $this->lang->line('purchase_order') ?></a>
                            ...
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?asp echo $this->lang->line('purchase_order') ?></h4>                                                                               
                                    </div>
...
$('#parsley_reg #demo_total_amount').val($('#parsley_reg #total_amount').val());
   new_discount_amount();
    ...
    clear_inputs();
          ...
          $.bootstrapGrowl('<?asp echo $this->lang->line('Selling Price Must Less Than MRP price');?>', { type: "warning" });          
       $('#parsley_reg #mrp').focus();
        }
        }else{
...
<section id="add_new_order" class="container clearfix main_section">
                                             ...
                                             <label for="items" class="text-center" ><?asp echo $this->lang->line('items') ?></label>	
                                                ...
                                                <div class="col col-lg-1" style="padding:1px">
                                                   <div class="form_sep">
                                                                ...
                                                                <label for="mrp" class="text-center"  ><?asp echo $this->lang->line('mrp') ?></label>
                                                ...
                                                <div class="col col-lg-1" style="padding:1px;width: 125px;">
                                                   <div class="form_sep">
                                                                ...
                                                                <label for="sub_total" class="text-center"  ><?asp echo $this->lang->line('sub_total') ?></label>
                                      ...
                                      <div class="row">
                                          ...
                                          <div class="col col-sm-3" style="padding-top: 50px"  >
                                              ...
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_purchase_order()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
    ...
    <?asp echo form_close();?>
           ...
           <div id="footer_space">
              ...
              <?asp if($this->session->userdata['purchase_order_per']['approve']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                              $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('purchase_order');?>', { type: "warning" });
                      ...
                      for (i = 0; i < posnic.length-1; i++){
                          if(posnic[i].checked==true){                             
                              $.ajax({
                                url: '<?asp echo base_url() ?>/index.asp/purchase_order/purchase_order_approve',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value
                                ...
                                complete: function(response) {
                                    if(response['responseText']=='TRUE'){
                                           $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $this->lang->line('purchase_order') ?>  <?asp echo $this->lang->line('approved');?>', { type: "success" });
                                        $("#dt_table_tools").dataTable().fnDraw();
                                         ...
                                         $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $this->lang->line('is') ?>   <?asp echo $this->lang->line('already');?> <?asp echo $this->lang->line('approved');?>', { type: "warning" });
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission')." ".$this->lang->line('to')." ".$this->lang->line('approve')." ".$this->lang->line('purchase_order');?>', { type: "error" });                              
                                    }
                                    }
                            });
                    ...
                    function posnic_group_item_active(){
                     ...
                     var field=document.forms.items_form;
                      ...
                      for (i = 0; i < posnic.length-1; i++){
                          if(posnic[i].checked==true){                             
                              $.ajax({
                                url: '<?asp echo base_url() ?>index.asp/purchase_order/item_active',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value
     ...
     function posnic_delete(){
            <?asp if($this->session->userdata['purchase_order_per']['delete']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      for (i = 0; i < field.length; i++){
                          if(field[i].checked==true){
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('paruchase_order');?>', { type: "warning" });
                      }else{
             ...
             if(result){
                        ...
                        for (i = 0; i < posnic.length; i++){
                          ...
                          if(posnic[i].checked==true){ 
                              ...
                              $.ajax({
                                url: '<?asp echo base_url() ?>/index.asp/purchase_order/delete',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value
                      ...
                      <?asp }else{?>
                                   $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Delete')." ".$this->lang->line('purchase_order');?>', { type: "error" });                       
                           <?asp }
                        ?>
                      }
                    ...
                    function posnic_group_item_deactive(){
                     ...
                     var field=document.forms.items_form;
                      ...
                      for (i = 0; i < posnic.length-1; i++){
                          if(posnic[i].checked==true){                             
                                 $.ajax({
                                    url: '<?asp echo base_url() ?>index.asp/purchase_order/item_deactive',
                                    type: "POST",
                                    data: {
                                        guid: posnic[i].value
