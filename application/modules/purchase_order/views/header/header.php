...
<script type="text/javascript" charset="utf-8">
    var point=3;
          $(document).ready( function () {
        	 ...
        	 refresh_items_table();
                 $('#selected_item_table .dataTables_empty').html('<?asp echo $this->lang->line('please_select').' '.$this->lang->line('items')." ".$this->lang->line('for')." ".$this->lang->line('purchase_order') ?>');
                     $('#add_new_order').hide();
                              posnic_table();
           ...
           $('#dt_table_tools').dataTable({
                                      "bProcessing": true,
				      "bServerSide": true,
                                      "sAjaxSource": "<?asp echo base_url() ?>index.asp/purchase_order/data_table",
                                       aoColumns: [  
                ...
                data: {
                 ...
                 complete: function(response) {
                    if(response['responseText']=='TRUE'){
                           $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $this->lang->line('purchase_order') ?>  <?asp echo $this->lang->line('deleted');?>', { type: "error" });
        ...
        <?asp if($this->session->userdata['purchase_order_per']['approve']==1){ ?>
           ...
           function edit_function(guid){
                        ...
                        <?asp if($this->session->userdata['purchase_order_per']['edit']==1){ ?>
                            ...
                            $('#loading').modal('show');
                            $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/purchase_order/get_purchase_order/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                        ...
                        <?asp }else{?>
                                 $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Edit')." ".$this->lang->line('purchase_order');?>', { type: "error" });                       
                        <?asp }?>
                       }
		</script>
