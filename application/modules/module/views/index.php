...
<script type="text/javascript">
     $(document).ready( function () {
         $('#add_new_module').click(function() { 
                <?asp if($this->session->userdata['module_per']['add']==1){ ?>
                var inputs = $('#add_module').serialize();
                if($('#parsley_reg').valid()){
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/module/add_module')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('module').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#add_module").trigger('reset');
                                       posnic_module_lists();
                                    }else  if(response['responseText']=='ALREADY'){
                                           $.bootstrapGrowl($('#module_name').val()+' <?asp echo $this->lang->line('module').' '.$this->lang->line('is_already_added')." ".$this->lang->line('should_not_repeat_order_number');?>', { type: "warning" });                           
                                    }else  if(response['responseText']=='FALSE'){
                                           $.bootstrapGrowl('<?asp echo $this->lang->line('Please Enter All Required Fields');?>', { type: "warning" });                           
                                    }else{
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('module');?>', { type: "error" });                           
                                    }
                       }
         ...
         $('#update_module').click(function() { 
                ...
                if($('#parsley_reg').valid()){
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/module/update_module')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                  if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('module').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       ...
                                       posnic_module_lists();
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Edit')." ".$this->lang->line('module');?>', { type: "error" });                           
                                    }
                       }
                 });
                 }
...
function posnic_assign(){
    <?asp if($this->session->userdata['module_per']['add']==1){ ?>
      ...
      $('#parent_modules').append('<div id="modules"/>');
       ...
       $.ajax({                                      
                             url: "<?asp echo base_url('index.asp/module/get_modules_list') ?>",                      
                             data: "", 
                             dataType: 'json',               
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_new" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $this->lang->line('new_module') ?></a>  
                        <a href="javascript:posnic_assign()" id="posnic_assign_module" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $this->lang->line('assign_module') ?></a>  
                        ...
                        <a href="javascript:posnic_module_lists()" class="btn btn-default" id="module_lists"><i class="icon icon-list"></i> <?asp echo $this->lang->line('module') ?></a>
                </div>
            </div>
    </div>
</nav>
                            ...
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?asp echo $this->lang->line('module') ?></h4>                                                                               
                                    </div>
                                    <table id="dt_table_tools" class="table-striped table-condensed" style="width: 100%"><thead>
                                        <tr>
                                          <th>Id</th>
                                          <th ><?asp echo $this->lang->line('select') ?></th>
                                          <th ><?asp echo $this->lang->line('module') ?></th>
                                       ...
                                       <div class="col col-lg-12" >
                                           <div class="row">
                                               ...
                                               <div class="col col-lg-10">
                                                    <div class="form_sep">
                                                         <label for="module_name" class="req"><?asp echo $this->lang->line('module_name') ?></label>                                                                                                       
                                                           <?asp $module_name=array('name'=>'module_name',
                               ...
                               <div class="row">
                                       <div class="col col-lg-12" >
                                           <div class="row">
                                               ...
                                               <div class="col col-lg-10">
                                                    <div class="form_sep">
                                                         <label for="module_name" ><?asp echo $this->lang->line('icon_class') ?></label>                                                                                                       
                    ...
                    <div class="row" id="add_button">
                                <div class="col-lg-4"></div>
                                  <div class="col col-lg-4 text-center"><br><br>
                                      <button id="add_new_module"  type="submit" name="save" class="btn btn-default"><i class="icon icon-save"> </i> <?asp echo $this->lang->line('save') ?></button>
                                      <a href="javascript:clear_add_module()" name="clear" id="clear_user" class="btn btn-default"><i class="icon icon-list"> </i> <?asp echo $this->lang->line('clear') ?></a>
                                  </div>
                              </div>
                <div class="row" id="update_button">
                        <div class="col-lg-4"></div>
                      <div class="col col-lg-4 text-center"><br><br>
                          <button id="update_module"  type="submit" name="save" class="btn btn-default"><i class="icon icon-save"> </i> <?asp echo $this->lang->line('update') ?></button>
                    ...
                    function posnic_delete(){
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('module');?>', { type: "warning" });
                      }else{
                            bootbox.confirm("<?asp echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('module') ?>", function(result) {
             if(result){
                        ...
                        for (i = 0; i < posnic.length; i++){
                          if(posnic[i].checked==true){                             
                              $.ajax({
                                ...
                                {
                                    if(response){
                                         $.bootstrapGrowl('<?asp echo $this->lang->line('module').' '. $this->lang->line('deleted');?>', { type: "success" });
                                        $("#dt_table_tools").dataTable().fnDraw();
                                    }
                                }
                            });
