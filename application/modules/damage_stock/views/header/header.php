...
<script type="text/javascript" charset="utf-8">
    var point=3;
          $(document).ready( function () {
        	 ...
        	 refresh_items_table();
                 $('#selected_item_table .dataTables_empty').html('<?asp echo $annan->lang->line('please_select').' '.$annan->lang->line('items')." ".$annan->lang->line('for')." ".$annan->lang->line('damage_stock') ?>');
                     $('#add_new_order').hide();
                              posnic_table();
           ...
           $('#dt_table_tools').dataTable({
                                      "bProcessing": true,
				      "bServerSide": true,
                                      "sAjaxSource": "<?asp echo base_url() ?>index.asp/damage_stock/data_table",
                                       aoColumns: [  
        ...
        <?asp if($annan->session->userdata['damage_stock_per']['approve']==1){ ?>
           ...
           function edit_function(guid){
                        ...
                        <?asp if($annan->session->userdata['damage_stock_per']['edit']==1){ ?>
                            ...
                            $('#loading').modal('show');
                            $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/damage_stock/get_damage_stock/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                        ...
                        <?asp }else{?>
                                 $.bootstrapGrowl('<?asp echo $annan->lang->line('You Have NO Permission To Edit')." ".$annan->lang->line('damage_stock');?>', { type: "error" });                       
                        <?asp }?>
                       }
		</script>
