...
<script type="text/javascript" charset="utf-8">
    var point=3;
          $(document).ready( function () {
        	 ...
        	 refresh_items_table();
                 $('#selected_item_table .dataTables_empty').html('<?php echo $this->lang->line('please_select').' '.$this->lang->line('items')." ".$this->lang->line('for')." ".$this->lang->line('damage_stock') ?>');
                     $('#add_new_order').hide();
                              posnic_table();
           ...
           $('#dt_table_tools').dataTable({
                                      "bProcessing": true,
				      "bServerSide": true,
                                      "sAjaxSource": "<?php echo base_url() ?>index.php/damage_stock/data_table",
                                       aoColumns: [  
        ...
        <?php if($this->session->userdata['damage_stock_per']['approve']==1){ ?>
           ...
           function edit_function(guid){
                        ...
                        <?php if($this->session->userdata['damage_stock_per']['edit']==1){ ?>
                            ...
                            $('#loading').modal('show');
                            $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/damage_stock/get_damage_stock/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                        ...
                        <?php }else{?>
                                 $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission To Edit')." ".$this->lang->line('damage_stock');?>', { type: "error" });                       
                        <?php }?>
                       }
		</script>
