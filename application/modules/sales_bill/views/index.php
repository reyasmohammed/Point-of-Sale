...
<script type="text/javascript">
   ...
   if($('#parsley_reg #order_date').value!=""){
              ...
              window.setTimeout(function ()
    {
         ...
         $.bootstrapGrowl('<?asp echo $this->lang->line('please_select')." ".$this->lang->line('delivery_note');?>', { type: "warning" }); 
         $('#parsley_reg #demo_order_number').select2('open');
    ...
    function save_new_order(){
         <?asp if($this->session->userdata['sales_bill_per']['add']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/sales_bill/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('sales_bill').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       ...
                                       refresh_items_table();
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('sales_bill');?>', { type: "error" });                           
                                    }
                       }
                });
                    }else{
                  
                     $.bootstrapGrowl('<?asp echo $this->lang->line('sales_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>', { type: "success" });                         
                     $('#parsley_reg #demo_order_number').select2('open');
                     $("#parsley_reg").trigger('reset');
                      $('#selected_item_table .dataTables_empty').html('<?asp echo $this->lang->line('please_select').' '.$this->lang->line('sales_order')." ".$this->lang->line('for')." ".$this->lang->line('sales_bill') ?>');
    ...
    function update_order(){
         <?asp if($this->session->userdata['sales_bill_per']['edit']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/sales_bill/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('sales_bill').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_sales_bill_lists();
                                       refresh_items_table();
        ...
        $('#parsley_reg #demo_order_number').change(function() {
             ...
             if($('#parsley_reg #demo_order_number').select2('data').type==1){
                   ...
                   var guid = $('#parsley_reg #demo_order_number').select2('data').sales_order;
                            ...
                            $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/sales_bill/get_sales_order/",                      
                             data: {
                                guid:guid,
                                sdn:sdn
                             },
                             ...
                             { 
                              ...
                              theNode.setAttribute('id','new_item_row_id_'+i)
                                  ...
                                  $.bootstrapGrowl('<?asp echo $this->lang->line('sales_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>', { type: "success" });                         
                                  $('#selected_item_table .dataTables_empty').html('<?asp echo $this->lang->line('sales_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>');
                                  }
...
console.log(guid);
                            ...
                            $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/sales_bill/get_direct_delivery_note/",                      
                             data: {
                                guid:guid
                             },
                                type:'POST',
          ...
          $('#parsley_reg #demo_order_number').select2({
              dropdownCssClass : 'supplier_select',
               formatResult: format_sales_order,
                ...
                placeholder: "<?asp echo $this->lang->line('search').' '.$this->lang->line('delivery_note') ?>",
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/sales_bill/search_sales_order',
...
refresh_items_table();
   $("#parsley_reg").trigger('reset');
    <?asp if($this->session->userdata['sales_bill_per']['add']==1){ ?>
            ...
            $("#parsley_reg #demo_order_number").select2('data', {id:'',text: '<?asp  echo $this->lang->line('search').' '.$this->lang->line('delivery_note')?>'});
             $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/sales_bill/order_number/",                      
                             data: "", 
                             dataType: 'json',               
                             success: function(data)        
                             {    
      ...
      <?asp }else{ ?>
                    $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('sales_quotation');?>', { type: "error" });                         
                    <?asp }?>
}
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_sales_bill" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $this->lang->line('addnew') ?></a>  
                      
                        <a href="" class="btn btn-default" id="deactive"  ><i class="icon icon-print"></i> <?asp echo $this->lang->line('print') ?></a>
                       
                        <a href="javascript:posnic_sales_bill_lists()" class="btn btn-default" id="sales_bill_lists"><i class="icon icon-list"></i> <?asp echo $this->lang->line('sales_bill') ?></a>
                            ...
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?asp echo $this->lang->line('sales_bill') ?></h4>                                                                               
                                    </div>
                                    <table id="dt_table_tools" class="table-striped table-condensed" style="width: 100%"><thead>
                                        <tr>
                                         <th>Id</th>
                                          <th ><?asp echo $this->lang->line('select') ?></th>
                                          <th ><?asp echo $this->lang->line('invoice') ?></th>
                                           <th style="width: 170px !important"><?asp echo $this->lang->line('grn_number') ?></th>
                                          
                                          <th><?asp echo $this->lang->line('company') ?></th>
                                           <th><?asp echo $this->lang->line('name') ?></th>
                                          <th><?asp echo $this->lang->line('order_date') ?></th>
...
<section id="add_new_order" class="container clearfix main_section">
    ...
    <div id="main_content" style="padding: 0 14px !important;">
        ...
        <input type="hidden" name="guid" id="guid" >
                         <div class="row">
                          <div class="panel panel-default">
                              <div class="panel-heading" >
                                     <h4 class="panel-title"><?asp echo $this->lang->line('sales_bill')." ".$this->lang->line('details') ?></h4>                                                                               
                               </div>
                                       ...
                                       <div id="" class="col col-sm-12" style="padding-right: 25px;padding-left: 25px">
                                           <div class="row">
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep supplier_select_2">
                                                        <label for="demo_order_number" ><?asp echo $this->lang->line('delivery_note') ?></label>													
                                                                  <?asp $demo_order_number=array('name'=>'demo_order_number',
                                                                                    'class'=>'  form-control',
                                                   ...
                                                   <div class="form_sep porchase_order_for_grn" style="margin-top:0px">
                                                         <label for="demo_order_number" ><?asp echo $this->lang->line('order_number') ?></label>	
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="dn_no" ><?asp echo $this->lang->line('dn_no') ?></label>													
                                                                     <?asp $round_off_amount=array('name'=>'demo_dn_no',
                                                                                        'class'=>'  form-control',
                                                                                        'id'=>'demo_dn_no',
                                                                                        'disabled'=>'disabled',
                                               ...
                                               <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="order_date" ><?asp echo $this->lang->line('dn_date') ?></label>													
                                           ...
                                           <div class="row">
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="customer_discount" ><?asp echo $this->lang->line('customer').' '.$this->lang->line('discount') ?> %</label>													
                                                 ...
                                                 <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="customer_discount_amount" ><?asp echo $this->lang->line('customer').' '.$this->lang->line('disc').' '.$this->lang->line('amt') ?></label>													
                                                 ...
                                                 <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="dn_no" ><?asp echo $this->lang->line('bill_no') ?></label>													
                                                                     <?asp $round_off_amount=array('name'=>'demo_bill_no',
                                                                                        'class'=>'  form-control',
                                                                                        'id'=>'demo_bill_no',
                                                                                        'disabled'=>'disabled',
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="bill_date" ><?asp echo $this->lang->line('bill_date') ?></label>													
                                                                     <div class="input-group date ebro_datepicker" data-date-format="dd.mm.yyyy" data-date-autoclose="true" data-date-start-view="2">
                                                                           <?asp $bill_date=array('name'=>'bill_date',
                                                                                            'class'=>'required form-control',
                                                                                            'id'=>'bill_date',
                    ...
                    <div class="col col-lg-9">
                        ...
                        <div class="row" ><input type="hidden" value="0" id='sl_number'>
                            ...
                            <div class="image_items">
                                ...
                                <table id='selected_item_table' class="table table-striped dataTable ">
                                    ...
                                    <th><?asp echo $this->lang->line('delivered_quty') ?></th>
                        ...
                        <div class="row" style="margin-left: 5px">
                                     ...
                                     <h4 class="panel-title"><?asp echo $this->lang->line('amount') ?></h4>                                                                               
                                                        ...
                                                        <label for="total_item_discount_amount" ><?asp echo $this->lang->line('total_item_discount_amount') ?></label>													
                                                         ...
                                                         <div class="form_sep " style="padding: 0 25px">
                                                        <label for="total_tax" ><?asp echo $this->lang->line('total_tax') ?></label>													
                                          ...
                                          <div class="col col-sm-6"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_sales_order()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_sales_order()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
    ...
    <script type="text/javascript">
        function posnic_group_approve(){
              <?asp if($this->session->userdata['sales_bill_per']['approve']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                              $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('sales_quotation');?>', { type: "warning" });
                      ...
                      for (i = 0; i < posnic.length-1; i++){
                          ...
                          if(posnic[i].checked==true){                             
                              $.ajax({
                                url: '<?asp echo base_url() ?>/index.asp/sales_bill/good_receiving_note_approve',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value,
                                    ...
                                    if(response['responseText']=='TRUE'){
                                       ...
                                       $("#dt_table_tools").dataTable().fnDraw();
                                        ...
                                        $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $this->lang->line('is')." ".$this->lang->line('already')." ".$this->lang->line('approved');?>', { type: "warning" });
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission')." ".$this->lang->line('to')." ".$this->lang->line('approve')." ".$this->lang->line('sales_bill');?>', { type: "error" });                       
                                   }
                               }
                            });
    ...
    function grn_group_delete(){
                     <?asp if($this->session->userdata['sales_bill_per']['delete']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      for (i = 0; i < field.length; i++){
                          if(field[i].checked==true){
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('sales_bill');?>', { type: "warning" });
                      }else{
                            bootbox.confirm("<?asp echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('sales_bill') ?>", function(result) {
             if(result){
                        ...
                        for (i = 0; i < posnic.length; i++){
                          if(posnic[i].checked==true){   
                              var guid=posnic[i].value;
                              $.ajax({
                                url: '<?asp echo base_url() ?>/index.asp/sales_bill/delete',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value
                                 ...
                                 complete: function(response) {
                                    if(response['responseText']=='TRUE'){
                                           $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?asp echo $this->lang->line('sales_bill') ?>  <?asp echo $this->lang->line('deleted');?>', { type: "error" });
                                        $("#dt_table_tools").dataTable().fnDraw();
                      ...
                      <?asp }else{?>
                               $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Delete')." ".$this->lang->line('sales_bill');?>', { type: "error" });                       
                       <?asp }
                    ?>
                      }
