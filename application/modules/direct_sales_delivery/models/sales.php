...
class Sales extends CI_Model{
    ...
    function get($end,$start,$like,$branch){
                $annan->db->select('direct_sales_delivery.* ,customers.guid as s_guid,customers.first_name as s_name,customers.company_name as c_name');
             
                $annan->db->from('direct_sales_delivery')->where('direct_sales_delivery.branch_id',$branch)->where('direct_sales_delivery.delete_status',0);
                $annan->db->join('customers', 'customers.guid=direct_sales_delivery.customer_id','left');
    ...
    function count($branch){
        $annan->db->select()->from('direct_sales_delivery')->where('branch_id',$branch)->where('active_status',1)->where('delete_status',0);
        $sql=  $annan->db->get();
        return $sql->num_rows();
    }
    ...
    function search_items($search){
     ...
     function get_direct_sales_delivery($guid){
         $annan->db->select('items.uom,items.no_of_unit,items.tax_Inclusive ,tax_types.type as tax_type_name,taxes.value as tax_value,taxes.type as tax_type,customers.guid as c_guid,customers.first_name as s_name,customers.company_name as c_name,customers.address as address,direct_sales_delivery.*,direct_sales_delivery_x_items.quty ,direct_sales_delivery_x_items.stock_id ,direct_sales_delivery_x_items.discount as item_discount,direct_sales_delivery_x_items.price,direct_sales_delivery_x_items.guid as o_i_guid ,items.guid as i_guid,items.name as items_name,items.code as i_code')->from('direct_sales_delivery')->where('direct_sales_delivery.guid',$guid);
         $annan->db->join('direct_sales_delivery_x_items', "direct_sales_delivery_x_items.direct_sales_delivery_id = direct_sales_delivery.guid  ",'left');
         $annan->db->join('items', "items.guid=direct_sales_delivery_x_items.item AND direct_sales_delivery_x_items.direct_sales_delivery_id='".$guid."' ",'left');
         $annan->db->join('taxes', "items.tax_id=taxes.guid AND items.guid=direct_sales_delivery_x_items.item  ",'left');
         $annan->db->join('tax_types', "taxes.type=tax_types.guid AND items.tax_id=taxes.guid AND items.guid=direct_sales_delivery_x_items.item  ",'left');
         $annan->db->join('customers', "customers.guid=direct_sales_delivery.customer_id AND direct_sales_delivery_x_items.direct_sales_delivery_id='".$guid."'  ",'left');
         $sql=  $annan->db->get();
         $data=array();
         foreach($sql->result_array() as $row){
     ...
     function delete_order_item($guid){      
          ...
          $annan->db->delete('direct_sales_delivery_x_items');
     }
     function approve_order($guid){
         $annan->db->where('guid',$guid);
         $annan->db->update('direct_sales_delivery',array('order_status'=>1));
         $annan->db->select()->from('direct_sales_delivery_x_items')->where('direct_sales_delivery_id',$guid);
         $sql=  $annan->db->get();
         foreach ($sql->result() as $row){
            $quty;
            $stock_id;
     ...
     function  check_approve($guid){
          $annan->db->select()->from('direct_sales_delivery')->where('guid',$guid)->where('order_status',1);
            $sql=  $annan->db->get();
            if($sql->num_rows()>0){
               return FALSE;
            }else{
     ...
     function add_direct_sales_delivery($guid,$item,$quty,$stock,$discount,$i){
         ...
         $annan->db->insert('direct_sales_delivery_x_items',array('stock_id'=>$stock,'guid'=>  md5($i.$guid.$item),'discount'=>$discount,'price'=>$price,'item'=>$item,'quty'=>$quty,'direct_sales_delivery_id'=>$guid));
         
               
     }
     function update_direct_sales_delivery($guid,$quty){
         $annan->db->where('guid',$guid);
         $annan->db->update('direct_sales_delivery_x_items',array('quty'=>$quty));
     }
