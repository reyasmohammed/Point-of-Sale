...
<script type="text/javascript">
   ...
   if($('#parsley_reg #order_date').value!=""){
    ...
    function save_new_grn(){
         <?asp if($this->session->userdata['goods_receiving_note_per']['add']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/goods_receiving_note/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('goods_receiving_note').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       ...
                                       refresh_items_table();
                                          ...
                                          $.bootstrapGrowl('<?asp echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('goods_receiving_note');?>', { type: "error" });                           
                                    }
                       }
                });
                    }else{
                  
                     $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>', { type: "success" });                         
                     $('#parsley_reg #demo_order_number').select2('open');
                     $("#parsley_reg").trigger('reset');
    ...
    function update_order(){
         <?asp if($this->session->userdata['goods_receiving_note_per']['edit']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?asp echo base_url('index.asp/goods_receiving_note/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?asp echo $this->lang->line('goods_receiving_note').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_goods_receiving_note_lists();
                                       refresh_items_table();
     ...
     $(document).ready( function () {
          ...
          $('#parsley_reg #items').change(function() {
              if(document.getElementById('new_item_row_id_'+$('#parsley_reg #items').select2('data').id) && $('#parsley_reg #diabled_item').val()!=$('#parsley_reg #items').select2('data').id){
                     $.bootstrapGrowl('<?asp echo $this->lang->line('this item already added');?> '+$('#parsley_reg #demo_order_number').val(), { type: "warning" });  
                       $('#parsley_reg #items').select2('open');
              }else{
    ...
    return  "<p >"+sup.text+"<img src='<?asp echo base_url() ?>/uploads/items/"+sup.image+"' style='float:right;height:59px'></img></p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.value+"</p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.category+"</p> <p style='width:130px;  margin-left: 218px'> "+sup.brand+"</p><p style='width:120px;  margin-left: 380px;margin-top: -28px;'> "+sup.department+"</p>";
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/goods_receiving_note/search_items/',
            ...
            if($('#parsley_reg #demo_order_number').select2('data').expired==0){
             ...
             $('#loading').modal('show');
                   var guid = $('#parsley_reg #demo_order_number').select2('data').id;
                            ...
                            $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/goods_receiving_note/get_purchase_order/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                             ...
                             if(data[0]['grn_status']==0){
                              ...
                              theNode.setAttribute('id','new_item_row_id_'+i)
                                  ...
                                  $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>', { type: "success" });                         
                                  $('#selected_item_table .dataTables_empty').html('<?asp echo $this->lang->line('purchase_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>');
                                  }
                             } 
                             else{
                               $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('all_items_was_received') ?>', { type: "success" });                         
                      ...
                      window.setTimeout(function ()
                    {
                     ...
                     $('#parsley_reg #demo_order_number').select2('open');
                     $("#parsley_reg").trigger('reset');
                     $.bootstrapGrowl('<?asp echo $this->lang->line('purchase_order')?> '+$('#parsley_reg #demo_order_number').select2('data').text+' <?asp echo $this->lang->line('was_expired');?>', { type: "error" });                         
                     $('#grn_no').val(grn_number);
                     $('#demo_grn_no').val(grn_number);
          ...
          $('#parsley_reg #demo_order_number').select2({
              dropdownCssClass : 'supplier_select',
               formatResult: format_purchase_order,
                ...
                ajax: {
                     url: '<?asp echo base_url() ?>index.asp/goods_receiving_note/search_purchase_order',
                     data: function(term, page) {
                            return {types: ["exercise"],
                                limit: -1,
                                term: term
   ...
   $("#parsley_reg").trigger('reset');
    <?asp if($this->session->userdata['goods_receiving_note_per']['add']==1){ ?>
            ...
            $("#parsley_reg #demo_order_number").select2('data', {id:'',text: 'Search PO'});
             $.ajax({                                      
                             url: "<?asp echo base_url() ?>index.asp/goods_receiving_note/order_number/",                      
                             data: "", 
                             dataType: 'json',               
                             success: function(data)        
                             {    
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_goods_receiving_note" class="btn btn-default" ><i class="icon icon-user"></i> <?asp echo $this->lang->line('addnew') ?></a>  
                        ...
                        <a href="javascript:posnic_goods_receiving_note_lists()" class="btn btn-default" id="goods_receiving_note_lists"><i class="icon icon-list"></i> <?asp echo $this->lang->line('goods_receiving_note') ?></a>
                            ...
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?asp echo $this->lang->line('goods_receiving_note') ?></h4>                                                                               
                                    </div>
                                    <table id="dt_table_tools" class="table-striped table-condensed" style="width: 100%"><thead>
                                        <tr>
                                         <th>Id</th>
                                          <th ><?asp echo $this->lang->line('select') ?></th>
                                          <th ><?asp echo $this->lang->line('purchase_order') ?></th>
    ...
    <div id="main_content" style="padding: 0 14px !important;">
        ...
        <input type="hidden" name="grn_guid" id="grn_guid" >
                         <div class="row">
                          <div class="panel panel-default">
                              <div class="panel-heading" >
                                     <h4 class="panel-title"><?asp echo $this->lang->line('goods_receiving_note')." ".$this->lang->line('details') ?></h4>                                                                               
                               </div>
                                       ...
                                       <div id="" class="col col-sm-12" style="padding-right: 25px;padding-left: 25px">
                                           ...
                                           <div class="row">
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="grn_no" ><?asp echo $this->lang->line('grn_no') ?></label>													
                                                                     <?asp $round_off_amount=array('name'=>'demo_grn_no',
                                                                                        'class'=>'  form-control',
                                                                                        'id'=>'demo_grn_no',
                                                                                        'disabled'=>'disabled',
                                                ...
                                                <div class="col col-sm-2" >
                                                   <div class="form_sep">
                                                            <label for="grn_date" ><?asp echo $this->lang->line('grn_date') ?></label>													
                                                                     <div class="input-group date ebro_datepicker" data-date-format="dd.mm.yyyy" data-date-autoclose="true" data-date-start-view="2">
                                                                           <?asp $grn_date=array('name'=>'grn_date',
                                                                                            'class'=>'required form-control',
                                                                                            'id'=>'grn_date',
                      ...
                      <div class="row" ><input type="hidden" value="0" id='sl_number'>
                            ...
                            <div class="image_items">
                                ...
                                <table id='selected_item_table' class="table table-striped dataTable ">
                                    ...
                                    <th><?asp echo $this->lang->line('ordered_quty') ?></th>
                                    <th><?asp echo $this->lang->line('received_quty') ?></th>
                                    
                                    <th><?asp echo $this->lang->line('ordered_free') ?></th>
                                    <th><?asp echo $this->lang->line('received_free') ?></th>
                                   
                                    
                                   
                                  
                                    <th><?asp echo $this->lang->line('quantity') ?></th>
                                    <th><?asp echo $this->lang->line('free') ?></th>
                                     <th><?asp echo $this->lang->line('tax') ?></th>
                                     <th><?asp echo $this->lang->line('discount') ?></th>
                                     <th><?asp echo $this->lang->line('amount') ?></th>
                                      ...
                                      <div class="row">
                                          <div class="col col-sm-3" style="padding-top: 50px" >
                                              <div class="form_sep " id="save_button" style="padding-left: 50px">
                                                       <label for="" >&nbsp;</label>	
                                                       <a href="javascript:save_new_grn()" class="btn btn-default"  ><i class="icon icon-save"></i> <?asp echo " ".$this->lang->line('save') ?></a>
                                                  </div>
                                          ...
                                          <div class="col col-sm-3" style="padding-top: 50px"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_goods_receiving_note()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_goods_receiving_note()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?asp echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
        ...
        function posnic_group_approve(){
              <?asp if($this->session->userdata['goods_receiving_note_per']['approve']==1){ ?>
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                              $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('goods_receiving_note');?>', { type: "warning" });
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?asp echo $this->lang->line('Select Atleast One')."".$this->lang->line('goods_receiving_note');?>', { type: "warning" });
                            ...
                            bootbox.confirm("<?asp echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('goods_receiving_note') ?>", function(result) {
             if(result){
